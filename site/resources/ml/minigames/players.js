...
async function fetchPlayersData(minigame) {
    try {
        const response = await fetch(`/api/v2/ml/boss-minigame-toplist-users?minigame=${minigame}`);
...
function sortPlayersDataInit(skill, ascending) {
    ...
    playersData.sort((a, b) => {
        const valueA = (a.MinigameGains && a.MinigameGains[skill]) || 0; // Ensuring the MinigameGains object exists
        const valueB = (b.MinigameGains && b.MinigameGains[skill]) || 0;
    ...
    top3.forEach(player => console.log(`${player.Username || 'Unnamed'}: ${player.MinigameGains[skill] || 0}`));

    console.log(`Bottom 3 players for skill ${skill}:`);
    bottom3.forEach(player => console.log(`${player.Username || 'Unnamed'}: ${player.MinigameGains[skill] || 0}`));
...
function createPlayersRow(item) {
    ...
    minigameGainsContainer.className = 'skills-container wrapped';
